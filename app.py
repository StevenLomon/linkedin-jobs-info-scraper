...
def scrape_linkedin(linkedin_url, num_results):
    df = pd.DataFrame(columns=['Förnamn', 'PosterId'])

    keyword = re.search(r'keywords=([^&]+)', linkedin_url).group(1)
    url = f"https://www.linkedin.com/voyager/api/voyagerJobsDashJobCards?decorationId=com.linkedin.voyager.dash.deco.jobs.search.JobSearchCardsCollectionLite-63&count={num_results}&q=jobSearch&query=(origin:HISTORY,keywords:{keyword},locationUnion:(geoId:105117694),selectedFilters:(distance:List(25.0)),spellCorrectionEnabled:true)&servedEventEnabled=false&start=0"
    ...
    for job in all_jobs:
        posterId = job['jobCardUnion']['jobPostingCard']['jobPosting'].get('posterId', None)
        new_row = pd.DataFrame({'Förnamn': 'Bob', 'PosterId': posterId}, index=[0])
...
linkedin_url = st.text_input('Enter LinkedIn Job URL:', '')
result_name = st.text_input('Enter a name for the csv:', '')
number_of_results = st.text_input('Enter amount of results you want to scrape:', '')

# Button to generate CSV
if st.button('Generate CSV'):
    if linkedin_url:
        scraped_data_df = scrape_linkedin(linkedin_url, number_of_results)
