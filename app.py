...
async def fetch_job_posting_ids(keyword, batch, session, max_retries=3, delay=1):
    start, stop = batch
...
async def extract_all_job_posting_ids(keyword, batches):
    async with aiohttp.ClientSession() as session:
        tasks = [fetch_job_posting_ids(session, keyword, batch) for batch in batches]
...
async def main(job_posting_ids, total_results, employee_threshold, less_than_keywords, more_than_keywords, semaphore_value=10):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for job_posting in job_posting_ids:
            ...
            task = extract_full_name_bio_and_linkedin_url(job_posting, sem, session)
...
if st.button('Generate File'):
    if linkedin_job_url:
        ...
        results = asyncio.run(main(all_job_posting_ids))
